...
type limiter struct {
	...
	// cache line size - state pointer size = 64 - 8; created to avoid false sharing.
	padding [56]byte //nolint
...
func New(rate int64, opts ...ratelimit.Option) ratelimit.RateLimit {
	return newLimit(rate, opts...)
}

func newLimit(rate int64, opts ...ratelimit.Option) *limiter {
